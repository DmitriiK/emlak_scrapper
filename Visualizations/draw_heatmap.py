...
def load_prices():
    ...
    try:
        ...
        read_emlak_sql = "SELECT  eml.id, eml.room, eml.is_furnished, eml.price, eml.sqm_netsqm, " \
                         ...
                         "FROM public.f_emlak eml WHERE eml.price/ " \
                         "eml.sqm_netsqm between 40 and 500 and eml.room between 1 and 5"
        ...
        while output is not None:
            print(output)
            row = cursor.fetchone()
            rent, bedrooms, apt_id, lon, lat = (int(row[5]), int(row[0]), row[1], float(row[6]), float(row[7]))
...
def start():
    print("loading data...")
    priced_points, slope, y_intercept = load_prices()
    ...
    out_fname = "output.phantom." + str(MAX_X)
